...
func defaultBuiltinSpecs() []Spec {
	return []Spec{
		...
		withBuiltinSources(Spec{
			...
			HeadlessArgs:          []string{"--print-timeout", "30m", "--dangerously-skip-permissions", "--add-dir", "{root}", "--print", "{prompt}"},
			InteractivePromptMode: InteractivePromptNone,
			InteractiveInvoke:     InteractiveInvokePrintOnly,
			InteractivePollMS:     DefaultInteractivePollMS,
			...
			ExternalBackend:       ExternalHosted,
			Telemetry:             "unknown",
			Notes:                 "Antigravity CLI replacement for Gemini; current agy help exposes no model or thinking flags",
		}),
		withBuiltinSources(Spec{
			ID:                    "gemini",
