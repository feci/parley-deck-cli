...
func validateDocumentAwaiting(ideaSlug string, known, required []string, review bool, doc document) Summary {
	...
	// No required-section gate here (lean-organizer fix-up F1, review round-01 CRIT-1):
	// append-only signoffs remain the only machine-validated consensus gate
	// (COOPERATION.md: "append-only signoffs remain the only gate"; §15.6: "Only (a)
	// is machine-validated today"). protocol.RequiredConsensusSections feeds the
	// drafting prompt and the scaffold generator so the driver never instructs a
	// drafter to produce an artifact the protocol's own Phase-3 template rejects; it
...
// rather than retyped, so the scaffold cannot drift from the drafting prompt, which
// reads the same constant (lean-organizer A.4; fix-up F1 keeps prompt+scaffold as the
// constant's only consumers — no status gate reads it).
