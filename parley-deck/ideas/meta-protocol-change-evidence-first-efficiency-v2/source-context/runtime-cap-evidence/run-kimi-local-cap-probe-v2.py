...
import json,os,tempfile,subprocess,time,threading,http.server,hashlib,shutil,datetime
out=Path(__file__).resolve().parent/'kimi-local-cap-probe-v2';out.mkdir(exist_ok=False)
...
class Handler(http.server.BaseHTTPRequestHandler):
 ...
 def do_POST(self):
  raw=self.rfile.read(int(self.headers.get('Content-Length','0')));body=json.loads(raw);requests.append({'path':self.path,'body':body});payload={'id':'local-fixture','object':'chat.completion','created':0,'model':'local-cap-fixture','choices':[{'index':0,'message':{'role':'assistant','content':'LOCAL_FIXTURE_DONE'},'finish_reason':'stop'}],'usage':{'prompt_tokens':1,'completion_tokens':1,'total_tokens':2}}
  if body.get('stream'):
   self.send_response(200);self.send_header('Content-Type','text/event-stream');self.end_headers()
   chunk={'id':'local-fixture','object':'chat.completion.chunk','created':0,'model':'local-cap-fixture','choices':[{'index':0,'delta':{'role':'assistant','content':'LOCAL_FIXTURE_DONE'},'finish_reason':'stop'}],'usage':payload['usage']};self.wfile.write(('data: '+json.dumps(chunk)+'\n\ndata: [DONE]\n\n').encode())
  else:
